...
const App = () => {
  ...
  const [cards, setCards] = useState(['캘린더', '게시판', '자료실', '커뮤니티', '프로젝트', '채팅']); // 카드 데이터
  ...
  const handleMenuClick = (index) => {
    if (activeMenu === index) {
      ...
      const reorderedCards = [selectedCard, ...cards.filter((_, i) => i !== index)];
  ...
  return (
    <div lang="ko">
      <header className="header" data-aos="fade-down" data-aos-duration="1500">
        <div className="header-top">
          <div className="announcement" data-aos="fade-up" data-aos-delay="100">
            <span className="new-tag">New</span> Lion 오피스 <strong>10인 미만 완전 무료! </strong> 지금 바로 시작하세요!
          </div>
        </div>
        <div className="gnb_pc inner_header">
          <ul className="list_gnb">
            <li className="logo-container" data-aos="zoom-in" data-aos-delay="100">
              <img src={logoImg} alt="LION 로고" className="header-logo" />
      ...
      <main>
        <section className="top floating-section" id="scroll_sectionTop" data-aos="fade-up">
          <div className="title">
            <h2>
          ...
          <div className="lion-logo-container" data-aos="fade-up" data-aos-delay="400">
            <img src={logoImg} alt="LION 로고" className="lion-logo" />
            <h3 className="lion-subtitle">LOTTE INFORMATION & ONLINE NETWORK</h3>
          </div>
        </section>

        <section className="feature" id="main_feature" data-aos="fade-up" data-aos-delay="200">
          <div className="title">
            <img src={bannerImg} alt="배너이미지" className="responsive-img" loading="lazy" />
            <h2 className="main-heading">Lion 오피스 하나면 <br className="m" /> 충분해요!</h2>
          </div>
          ...
          <div className={`feature_cards ${showAllCards ? 'show-all' : ''}`}>
            {cards.map((title, index) => (
              <div
                key={index}
                className={`card ${!showAllCards && index >= 4 ? 'hidden' : ''}`}
                data-aos={index % 2 === 0 ? 'fade-left' : 'fade-right'}
                data-aos-delay={300 + index * 100}
              >
                <h3>{title}</h3>
                <p>{title}에 대한 설명입니다.</p>
                <Link to={`/${title.toLowerCase()}`} className="detail-link">자세히 보기</Link>
              </div>
            ))}
          </div>
        </section>

        <section className="summary-section" data-aos="fade-up" data-aos-delay="600">
          <h2>LION 오피스가 더 특별한 이유</h2>
          <p className="summary-description">
            LION 오피스는 자체 개발 기능, 무상업그레이드 정책, 타 서비스와의 폭 넓은 체류로 차별성을 더합니다.
          </p>

          <div className="summary-cards">
            <div className="summary-card" data-aos="fade-left" data-aos-delay="100">
            ...
            <div className="summary-card" data-aos="fade-right" data-aos-delay="300">
            ...
            <div className="summary-card" data-aos="fade-up" data-aos-delay="500">
